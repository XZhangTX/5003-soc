...
@torch.no_grad()
def evaluate_model(model, loader, device, collect_attention: bool = False):
    model.eval()
    ys, ps = [], []
...
def main(args):
    ...
    run_dir = ensure_dir(Path(args.output_root) / "train" / (args.tag or timestamp()))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        metrics, y_true, y_pred, attention = evaluate_model(model, val_loader, device, collect_attention=True)
        row = {
            "epoch": epoch,
        ...
        pd.DataFrame({"y_true": y_true * 100.0, "y_pred": y_pred * 100.0}).to_csv(run_dir / "val_predictions.csv", index=False)
        plot_prediction_scatter(y_true * 100.0, y_pred * 100.0, run_dir / "val_scatter.png", title="Validation Prediction Scatter")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Train SOC model directly from aligned S11 CSV files")
