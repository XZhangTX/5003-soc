...
def _resolve_dc_modes(args):
    if getattr(args, "dc_modes", None):
        return list(args.dc_modes)
    return [args.dc_mode]
...
def _build_experiments(args):
    ...
    experiments = []
    for dc_mode in _resolve_dc_modes(args):
        dc_suffix = dc_mode.lower()
        experiments.extend(
            [
                {
                    "task": "soc",
                    ...
                    "model_key": "xgb",
                    "kind": "baseline_soc",
                    "tag": f"{args.tag}-soc-{dc_suffix}-xgb",
                    "cmd": [
                        sys.executable,
                        "baseline_tabular.py",
                        "--input-root",
                ...
                },
                {
                    ...
                    "model_key": "mlp",
                    "kind": "baseline_soc",
                    "tag": f"{args.tag}-soc-{dc_suffix}-mlp",
                    "cmd": [
                        sys.executable,
                        "baseline_tabular.py",
                        "--input-root",
                ...
                },
                {
                    ...
                    "kind": "train_soc",
                    "tag": f"{args.tag}-soc-{dc_suffix}-transformer",
                    "cmd": [
                        sys.executable,
                        "-u",
                        "-m",
                        ...
                        str(args.patience),
                        "--tag",
                        f"{args.tag}-soc-{dc_suffix}-transformer",
                ...
                },
                {
                    ...
                    "kind": "train_soc",
                    "tag": f"{args.tag}-soc-{dc_suffix}-convtransformer",
                    "cmd": [
                        sys.executable,
                        "-u",
                        "-m",
                        ...
                        str(args.patience),
                        "--tag",
                        f"{args.tag}-soc-{dc_suffix}-convtransformer",
                    ] + (["--pin-memory"] if args.pin_memory else []),
                },
                {
                    "task": "soh",
                    ...
                    "model_key": "xgb",
                    "kind": "baseline_soh",
                    "tag": f"{args.tag}-soh-{dc_suffix}-xgb",
                    "cmd": [
                        sys.executable,
                ...
                },
                {
                    ...
                    "model_key": "mlp",
                    "kind": "baseline_soh",
                    "tag": f"{args.tag}-soh-{dc_suffix}-mlp",
                    "cmd": [
                        sys.executable,
                        "baseline_tabular_soh_proxy.py",
                        "--input-root",
                ...
                },
                {
                    ...
                    "model_key": "transformer",
                    "kind": "train_soh",
                    "tag": f"{args.tag}-soh-{dc_suffix}-transformer",
                    "cmd": [
                        sys.executable,
                        "-u",
                        "-m",
                        ...
                        str(args.patience),
                        "--tag",
                        f"{args.tag}-soh-{dc_suffix}-transformer",
                    ] + (["--pin-memory"] if args.pin_memory else []),
                },
                {
                    "task": "soh",
                    ...
                    "model_key": "conv_transformer",
                    "kind": "train_soh",
                    "tag": f"{args.tag}-soh-{dc_suffix}-convtransformer",
                    "cmd": [
                        sys.executable,
                        "-u",
                        "-m",
                        ...
                        str(args.patience),
                        "--tag",
                        f"{args.tag}-soh-{dc_suffix}-convtransformer",
                    ] + (["--pin-memory"] if args.pin_memory else []),
                },
            ]
        )
...
def _summarize(experiments, output_root: Path, report_dir: Path):
    ...
    summary_df = pd.DataFrame(rows).sort_values(["task", "dc_mode", "rmse", "model"]).reset_index(drop=True)
    ...
    for task in ["SOC", "SOH"]:
        ...
        for dc_mode in ["all", "D", "C"]:
            ...
            if dc_df.empty:
                continue
            markdown_lines.append(f"### dc-mode = {dc_mode}")
            markdown_lines.append("")
...
def build_arg_parser():
    ...
    parser.add_argument("--dc-modes", nargs="*", choices=["all", "D", "C"], default=None)
