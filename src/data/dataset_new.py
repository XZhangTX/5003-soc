...
class S11AlignedDatasetNew(Dataset):
    def __init__(self, bundle: S11DatasetBundleNew):
        ...
        self.sample_meta = bundle.sample_meta.reset_index(drop=True)
...
def build_bundle_new(
    ...
    for record in records:
        ...
        row_meta = mag_df.loc[mask, list(META_COLUMNS)].copy()
        row_meta["day"] = record.day
        row_meta["series_name"] = record.series_name
        row_meta["record_id"] = record.record_id
        row_meta["row_index"] = np.flatnonzero(mask)
        row_meta = row_meta.rename(columns={"Cycle": "cycle", "DC": "dc", "SOC": "soc"})
        row_meta = row_meta[["record_id", "day", "series_name", "cycle", "dc", "soc", "row_index"]]

        x_parts.append(features)
        y_parts.append(soc)
        group_parts.append(groups)
        meta_parts.append(row_meta.reset_index(drop=True))
