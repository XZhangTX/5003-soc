...
from src.data import build_bundle, discover_s11_records
...
def main(args):
    ...
    metrics = regression_metrics(val_bundle.y, y_pred)

    out_dir = ensure_dir(Path(args.output_root) / "baseline" / f"{args.model}-{args.tag or timestamp()}")
    pd.DataFrame({"y_true": val_bundle.y * 100.0, "y_pred": y_pred * 100.0}).to_csv(out_dir / "predictions.csv", index=False)
    save_json(out_dir / "metrics.json", metrics)
    plot_prediction_scatter(val_bundle.y * 100.0, y_pred * 100.0, out_dir / "scatter.png", title=f"{args.model.upper()} Scatter")
    print(f"Saved baseline outputs to {out_dir}")


def build_arg_parser():
    parser = argparse.ArgumentParser(description="Train tabular baselines on aligned S11 CSV files")
